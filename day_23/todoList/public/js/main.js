...
async function createTodo(title) {
    try {
        const res = await createTodoAPI(title); // trả về 1 object
        todos.push(res.data) // push object trên vào mảng ban đầu
...
async function deleteTodo(id) {
    try {
        await deleteTodoAPI(id) // Gọi API xóa (xóa trên server)

        // Xóa todo trong mảng todos ban đầu (xóa trên todos)
